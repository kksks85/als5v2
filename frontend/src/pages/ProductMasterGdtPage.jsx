...
export default function ProductMasterGdtPage({ records, setRecords, masterName = 'GDT', idPrefix = 'gdt', columns = gdtColumns }) {
  ...
  if (selectedRecord) return <GdtRecordView record={selectedRecord} columns={columns} masterName={masterName} onBack={() => setSelectedRecord(null)} onEdit={() => { setEditingRecord(selectedRecord); setSelectedRecord(null) }} />
  ...
  return <>
    <div className="incident-list-head product-master-heading"><div className="incident-list-title"><h1>Product Master - {masterName}</h1><p>Material and item inventory register for {masterName}.</p></div><button className="compact-button secondary" onClick={exportTemplate}><Download size={15} /> Download Excel template</button></div>
...
function GdtRecordView({ record, columns, masterName, onBack, onEdit }) {
  return <section className="product-record-page"><header className="group-config-header"><div><button className="incident-back-button" onClick={onBack}><ArrowLeft size={15} /> Product Master - {masterName}</button><h1>{record.material_description || `${masterName} material record`}</h1><p>{record.part_number || 'Material details'}</p></div><div><button className="incident-cancel-button" onClick={onBack}>Close</button><button className="incident-submit-button" onClick={onEdit}>Edit material</button></div></header><section className="product-record-sheet">{columns.map(({ key, label }) => <div key={key}><span>{label}</span><strong>{record[key] || '--'}</strong></div>)}</section></section>
}
