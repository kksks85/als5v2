...
import { initialProcesses } from '../data/processConfiguration'

const emptyProcess = { repairExecution: '', status: '', order: '' }
const columns = [
...
export default function ProcessConfigurationPage() {
  ...
  const filteredProcesses = useMemo(() => processes.filter((process) => !search || [process.repairExecution, process.status, process.order].some((value) => String(value).toLowerCase().includes(search.toLowerCase()))), [processes, search])
  ...
  const exportCsv = () => {
    ...
    const data = [['ID', 'Repair Execution', 'Status', 'Order'].map(csv), ...filteredProcesses.map((p) => [p.id, p.repairExecution, p.status, p.order].map(csv))].map((r) => r.join(',')).join('\n')
...
function ProcessForm({ process, onCancel, onSubmit }) {
  const [form, setForm] = useState(process || emptyProcess)
  ...
  return <form className="customer-form-page" onSubmit={submit}>
    ...
    <section className="customer-form-sheet"><section className="customer-form-section"><h2>Process details</h2><div className="customer-form-grid"><label className={`customer-field ${errors.repairExecution ? 'has-error' : ''}`}><span>Repair execution {errors.repairExecution && <em>*</em>}</span><select value={form.repairExecution} onChange={(event) => update('repairExecution', event.target.value)}><option value="">Select repair execution</option><option>Repair at Factory</option><option>Repair at site</option><option>Repair at site - Vendor</option></select>{errors.repairExecution && <small>{errors.repairExecution}</small>}</label><label className={`customer-field ${errors.status ? 'has-error' : ''}`}><span>Status {errors.status && <em>*</em>}</span><input value={form.status} onChange={(event) => update('status', event.target.value)} placeholder="e.g. Work in Progress" />{errors.status && <small>{errors.status}</small>}</label><label className={`customer-field ${errors.order ? 'has-error' : ''}`}><span>Order {errors.order && <em>*</em>}</span><input type="number" min="1" value={form.order} onChange={(event) => update('order', event.target.value)} placeholder="Sequence number" />{errors.order && <small>{errors.order}</small>}</label></div></section></section>
