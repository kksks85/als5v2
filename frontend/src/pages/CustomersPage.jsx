...
export const initialCustomers = [
  { id: 1, name: 'Indian Air Force', number: 'TASL-CUST-001', address: 'Air Force Headquarters, Vayu Bhawan, New Delhi 110001', primaryContact: { name: 'Rajesh Kumar', designation: 'Technical Director', email: 'rajesh.kumar@indianairforce.in', phone: '+91 9010012345', rank: 'Group Captain', site: 'Head Office' }, contacts: [{ id: 1, name: 'Rajesh Kumar', designation: 'Technical Director', phone: '+91 9010012345', email: 'rajesh.kumar@indianairforce.in', site: 'Head Office', address: 'Main Complex, New Delhi, India' }, { id: 2, name: 'Suresh Patel', designation: 'Operations Manager', phone: '+91 9010012346', email: 'suresh.patel@indianairforce.in', site: 'Regional Office', address: 'Regional Hub, Bangalore, India' }, { id: 3, name: 'Rohit Verma', designation: 'Support Lead', phone: '+91 9010012347', email: 'rohit.verma@indianairforce.in', site: 'Support Center', address: 'Technical Support, Pune, India' }, { id: 4, name: 'Anil Bhat', designation: 'Maintenance Engineer', phone: '+91 9010012348', email: 'anil.bhat@indianairforce.in', site: 'Service Center', address: 'Maintenance Facility, Hyderabad, India' }] },
  { id: 2, name: 'Indian Army', number: 'TASL-CUST-002', address: 'Army Headquarters, New Delhi 110010', primaryContact: { name: 'Vikram Singh', designation: 'Chief Operations Officer', email: 'vikram.singh@indianarmy.in', phone: '+91 9011001234', rank: 'Colonel', site: 'Army HQ' }, contacts: [{ id: 1, name: 'Vikram Singh', designation: 'Chief Operations Officer', phone: '+91 9011001234', email: 'vikram.singh@indianarmy.in', site: 'Army HQ', address: 'New Delhi 110010' }, { id: 2, name: 'Priya Sharma', designation: 'Logistics Manager', phone: '+91 9011001235', email: 'priya.sharma@indianarmy.in', site: 'Agra Cantonment', address: 'Agra, Uttar Pradesh' }, { id: 3, name: 'Arjun Mehta', designation: 'Technical Lead', phone: '+91 9011001236', email: 'arjun.mehta@indianarmy.in', site: 'Delhi Cantonment', address: 'New Delhi' }] },
  { id: 3, name: 'Indian Navy', number: 'TASL-CUST-003', address: 'Naval Headquarters, South Block, New Delhi 110011', primaryContact: { name: 'Admiral Ramesh', designation: 'Chief Naval Operations', email: 'ramesh@indiannavy.in', phone: '+91 9012001234', rank: 'Rear Admiral', site: 'Naval HQ' }, contacts: [{ id: 1, name: 'Admiral Ramesh', designation: 'Chief Naval Operations', phone: '+91 9012001234', email: 'ramesh@indiannavy.in', site: 'Naval HQ', address: 'New Delhi 110011' }, { id: 2, name: 'Captain Desai', designation: 'Fleet Commander', phone: '+91 9012001235', email: 'desai@indiannavy.in', site: 'Naval Base Kochi', address: 'Kochi, Kerala' }, { id: 3, name: 'Commander Nair', designation: 'Support Services', phone: '+91 9012001236', email: 'nair@indiannavy.in', site: 'Naval Base Mumbai', address: 'Mumbai, Maharashtra' }, { id: 4, name: 'Lt. Commander Pillai', designation: 'Communications', phone: '+91 9012001237', email: 'pillai@indiannavy.in', site: 'Air Station Kochi', address: 'Kochi, Kerala' }, { id: 5, name: 'Commander Bhat', designation: 'Operations', phone: '+91 9012001238', email: 'bhat@indiannavy.in', site: 'Eastern Naval', address: 'Visakhapatnam, Andhra Pradesh' }] },
  { id: 4, name: 'Indian Army Special Forces', number: 'TASL-CUST-004', address: 'Special Forces Command, Delhi Cantonment, New Delhi 110010', primaryContact: { name: 'Colonel Rao', designation: 'Command Head', email: 'rao@specialforces.in', phone: '+91 9013001234', rank: 'Colonel', site: 'Command Centre' }, contacts: [{ id: 1, name: 'Colonel Rao', designation: 'Command Head', phone: '+91 9013001234', email: 'rao@specialforces.in', site: 'Command Centre', address: 'Delhi Cantonment, New Delhi' }, { id: 2, name: 'Major Singh', designation: 'Training Head', phone: '+91 9013001235', email: 'singh.major@specialforces.in', site: 'Mountain Training', address: 'Auli, Uttarakhand' }, { id: 3, name: 'Captain Patel', designation: 'Operations Officer', phone: '+91 9013001236', email: 'patel.captain@specialforces.in', site: 'Commando Base', address: 'Agra Cantonment, Uttar Pradesh' }] },
]
...
function CustomerForm({ customer, onCancel, onSubmit }) {
  ...
  const [newContact, setNewContact] = useState({ name: '', designation: '', phone: '', email: '', site: '', address: '' })
  ...
  const addContact = () => {
    ...
    setNewContact({ name: '', designation: '', phone: '', email: '', site: '', address: '' })
    setEditingContactId(null)
  }
  const removeContact = (id) => setForm((current) => ({ ...current, contacts: current.contacts.filter(c => c.id !== id) }))
  const editContact = (contact) => {
    setEditingContactId(contact.id)
    setNewContact({ name: contact.name || '', designation: contact.designation || '', phone: contact.phone || '', email: contact.email || '', site: contact.site || '', address: contact.address || '' })
  }
  ...
  return <form className="customer-form-page" onSubmit={submit}>
    ...
    <section className="customer-form-sheet">
      ...
      <section className="customer-form-section"><h2>Additional contacts ({form.contacts.length})</h2>{form.contacts.length > 0 && <div className="contacts-table-wrapper"><table className="contacts-table"><thead><tr><th>Contact Name</th><th>Designation</th><th>Phone Number</th><th>Email</th><th>Site Name</th><th>Site Address</th><th>Actions</th></tr></thead><tbody>{form.contacts.map((contact) => <tr key={contact.id}><td>{contact.name}</td><td>{contact.designation}</td><td>{contact.phone}</td><td className="truncate">{contact.email}</td><td>{contact.site}</td><td className="truncate">{contact.address}</td><td className="contact-actions"><button type="button" className="icon-button" title="Edit contact" onClick={() => editContact(contact)}><Edit2 size={14} /></button><button type="button" className="icon-button danger" title="Delete contact" onClick={() => removeContact(contact.id)}><Trash2 size={14} /></button></td></tr>)}</tbody></table></div>}
      <div className="add-contact-section"><h3>{editingContactId ? 'Edit Contact' : 'Add New Contact'}</h3><div className="customer-form-grid"><label className="customer-field"><span>Contact Name *</span><input value={newContact.name} onChange={(e) => setNewContact({...newContact, name: e.target.value})} placeholder="Contact name" /></label><label className="customer-field"><span>Designation</span><input value={newContact.designation} onChange={(e) => setNewContact({...newContact, designation: e.target.value})} placeholder="e.g., Project Manager" /></label><label className="customer-field"><span>Phone Number</span><input value={newContact.phone} onChange={(e) => setNewContact({...newContact, phone: e.target.value})} placeholder="Phone number" /></label><label className="customer-field"><span>Email</span><input value={newContact.email} onChange={(e) => setNewContact({...newContact, email: e.target.value})} type="email" placeholder="Email address" /></label><label className="customer-field"><span>Site Name</span><input value={newContact.site} onChange={(e) => setNewContact({...newContact, site: e.target.value})} placeholder="Site name" /></label><label className="customer-field"><span>Site Address</span><input value={newContact.address} onChange={(e) => setNewContact({...newContact, address: e.target.value})} placeholder="Site address" /></label></div><div className="contact-form-actions"><button type="button" className="add-contact-btn" onClick={addContact}>{editingContactId ? 'Save Contact' : 'Add Contact'}</button>{editingContactId && <button type="button" className="customer-cancel-button" onClick={cancelContactEdit}>Cancel</button>}</div></div></section>
    </section>
