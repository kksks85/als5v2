...
export default function ProductCategoryPage({ category, assets, products, contracts, currentUser, selectedCustomer, onUpdateAsset, onDeleteAsset, initialSerialNumbers = [] }) {
  ...
  return <section className="product-category-page">
    ...
    {viewingAsset && <AssetDetail asset={viewingAsset} products={products} onClose={() => setViewingAsset(null)} onEdit={() => { setViewingAsset(null); setEditingAsset(viewingAsset) }} />}
  </section>
}
...
function AssetForm({ asset, contracts, currentUser, onClose, onSave }) {
  ...
  if (activeTab === 'flight-data') return <div className="report-dialog-backdrop"><form className="asset-record-form asset-flight-record-form" onSubmit={submitForm}>
  ...
  return <div className="report-dialog-backdrop"><form className="asset-record-form" onSubmit={submitForm}>
    <header><div><p>Product lifecycle</p><h2>{asset.serialNumber}</h2><span>{asset.category} · Edit mode</span></div><button type="button" onClick={onClose} aria-label="Close asset form"><X size={17} /></button></header>
    <div className="asset-detail-tabs asset-edit-tabs" role="tablist" aria-label={`${asset.serialNumber} edit views`}><button type="button" className="active" role="tab" aria-selected={true}>Overview</button><button type="button" role="tab" aria-selected={false} onClick={() => setActiveTab('flight-data')}>Flight Data</button></div>
...
function AssetDetail({ asset, products, onClose, onEdit }) {
  const [activeTab, setActiveTab] = useState('overview')
  ...
  const productComponents = useMemo(() => products.filter((product) => product.product_serial_number === asset.serialNumber), [asset.serialNumber, products])
  ...
  if (activeTab === 'pre-delivery-flight') return <div className="report-dialog-backdrop asset-detail-backdrop"><section className="asset-detail-dialog" role="dialog" aria-modal="true" aria-label={`${asset.serialNumber} product record`}>
